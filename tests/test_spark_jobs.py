...
def test_trip_aggregate(spark) -> None:
    ...
    rows = [
        (1, datetime(2024, 1, 1, 8, 0), datetime(2024, 1, 1, 8, 15), 1, 2.0, 1, "N", 132, 236, 1, 10.0, 0, 0, 1.0, 0, 0, 12.0, 0, 0),
        (1, datetime(2024, 1, 1, 8, 30), datetime(2024, 1, 1, 8, 50), 1, 4.0, 1, "N", 132, 100, 1, 20.0, 0, 0, 2.0, 0, 0, 24.0, 0, 0),
    ]
...
def test_revenue_aggregate(spark) -> None:
    ...
    rows = [
        (1, datetime(2024, 1, 1, 8, 0), datetime(2024, 1, 1, 8, 15), 1, 2.0, 1, "N", 132, 236, 1, 10.0, 0, 0, 1.0, 0, 0, 12.0, 0, 0),
        (1, datetime(2024, 1, 1, 9, 0), datetime(2024, 1, 1, 9, 20), 2, 3.0, 1, "N", 100, 200, 2, 15.0, 0, 0, 0.0, 0, 0, 16.0, 0, 0),
    ]
    df = spark.createDataFrame(rows, schema=YELLOW_TRIP_SCHEMA)
