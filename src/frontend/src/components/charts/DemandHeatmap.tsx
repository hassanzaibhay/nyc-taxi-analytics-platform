...
export default function DemandHeatmap({ data, maxZones = 30 }: Props) {
  ...
  data.forEach((d) => {
    if (!nameByZone.has(d.zone_id)) {
      nameByZone.set(d.zone_id, d.zone_name ?? `Zone ${d.zone_id}`);
    }
  });
    ...
    <div className="overflow-x-auto">
      <table className="border-separate border-spacing-1 text-xs">
        ...
        <tbody>
          {zones.map((z) => (
            <tr key={z}>
              <td className="text-right pr-2 text-slate-500 whitespace-nowrap max-w-[160px] truncate" title={nameByZone.get(z)}>
                {nameByZone.get(z)}
              </td>
              {Array.from({ length: 24 }, (_, h) => {
                ...
                return (
                  <td
                    key={h}
                    title={`${nameByZone.get(z)}, ${h}:00 → ${v.toFixed(1)} trips`}
